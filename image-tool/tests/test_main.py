...
@pytest.fixture(scope="module") # Use "module" scope for efficiency if test image creation is slow
def test_env():
...
def test_resize_success(test_env, capsys):
    ...
    image_tool_main.resize_image(test_env["input_image"], output_path, target_w, target_h)
    assert os.path.exists(output_path)
    with Image.open(output_path) as img:
        assert img.size == (target_w, target_h)
    captured = capsys.readouterr()
    assert f"Image resized successfully and saved to {output_path}" in captured.out
...
def test_resize_output_invalid_path(test_env, capsys):
    invalid_out_path = os.path.join(test_env["outputs_dir"], "invalid_dir/resized.png")
    image_tool_main.resize_image(test_env["input_image"], invalid_out_path, 5, 5)
    captured = capsys.readouterr()
...
def test_thumbnail_success(test_env, capsys):
    ...
    image_tool_main.create_thumbnail(test_env["input_image"], output_path, target_size)
    assert os.path.exists(output_path)
    with Image.open(output_path) as img:
        assert img.size == (expected_w, expected_h)
    captured = capsys.readouterr()
    assert f"Thumbnail created successfully and saved to {output_path}" in captured.out
...
def test_thumbnail_output_invalid_path(test_env, capsys):
    invalid_out_path = os.path.join(test_env["outputs_dir"], "invalid_dir/thumb.png")
    image_tool_main.create_thumbnail(test_env["input_image"], invalid_out_path, 5)
    captured = capsys.readouterr()
    assert "Error saving image to" in captured.out
    assert not os.path.exists(invalid_out_path)
...
def test_flip_horizontal_success(test_env, capsys):
    ...
    image_tool_main.flip_image(test_env["input_image"], output_path, "horizontal")
    assert os.path.exists(output_path)
    captured = capsys.readouterr()
    assert f"Image flipped horizontally and saved to {output_path}" in captured.out
...
def test_flip_vertical_success(test_env, capsys):
    ...
    assert os.path.exists(output_path)
    captured = capsys.readouterr()
    assert f"Image flipped vertically and saved to {output_path}" in captured.out
...
def test_flip_input_not_image(test_env, capsys): # Added this test for completeness
    output_path = get_output_path(test_env, "flip_not_img.png")
    image_tool_main.flip_image(test_env["not_an_image"], output_path, "horizontal")
    captured = capsys.readouterr()
    assert "Error: Cannot identify image file" in captured.out
    assert not os.path.exists(output_path)

def test_flip_output_invalid_path(test_env, capsys):
    invalid_out_path = os.path.join(test_env["outputs_dir"], "invalid_dir/flipped.png")
    image_tool_main.flip_image(test_env["input_image"], invalid_out_path, "horizontal")
    captured = capsys.readouterr()
    assert "Error saving image to" in captured.out
    assert not os.path.exists(invalid_out_path)
...
def test_crop_success(test_env, capsys):
    ...
    with Image.open(output_path) as img:
        assert img.size == (expected_w, expected_h)
    captured = capsys.readouterr()
    assert f"Image cropped successfully and saved to {output_path}" in captured.out
...
def test_crop_input_not_image(test_env, capsys): # Added this test for completeness
    output_path = get_output_path(test_env, "crop_not_img.png")
    image_tool_main.crop_image(test_env["not_an_image"], output_path, 0,0,5,5)
    captured = capsys.readouterr()
    assert "Error: Cannot identify image file" in captured.out
    assert not os.path.exists(output_path)

def test_crop_output_invalid_path(test_env, capsys):
    invalid_out_path = os.path.join(test_env["outputs_dir"], "invalid_dir/cropped.png")
    image_tool_main.crop_image(test_env["input_image"], invalid_out_path, 0,0,5,5)
    captured = capsys.readouterr()
    assert "Error saving image to" in captured.out
    assert not os.path.exists(invalid_out_path)
