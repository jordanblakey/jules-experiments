...
def resize_image(input_path, output_path, width, height):
    ...
    try:
        ...
        print(
            f"Error: Cannot identify image file. The file at {input_path} "
...
def flip_image(input_path, output_path, direction):
    ...
    try:
        if direction == "horizontal":
            ...
            print(
                f"Error: Invalid flip direction '{direction}'. "
...
def main():
    parser = argparse.ArgumentParser(
        description="A simple image manipulation tool."
    )
    ...
    resize_parser.add_argument(
        "input_file", help="Path to the input image file"
    )
    resize_parser.add_argument(
        "output_file", help="Path to save the resized image"
    )
    resize_parser.add_argument(
        "--width", type=int, required=True, help="New width for the image"
    )
    resize_parser.add_argument(
        "--height", type=int, required=True, help="New height for the image"
    )

    # Thumbnail command
    thumbnail_parser = subparsers.add_parser(
        "thumbnail", help="Create a thumbnail for an image"
    )
    thumbnail_parser.add_argument(
        "input_file", help="Path to the input image file"
    )
    thumbnail_parser.add_argument(
        "output_file", help="Path to save the thumbnail"
    )
    thumbnail_parser.add_argument(
        "--size",
        type=int,
        default=128,
        help="Max size of the thumbnail (default: 128x128)",
    )
    ...
    flip_parser.add_argument(
        "output_file", help="Path to save the flipped image"
    )
    flip_parser.add_argument(
        "--direction",
        choices=["horizontal", "vertical"],
        required=True,
        help="Direction to flip",
    )
    ...
    crop_parser.add_argument(
        "output_file", help="Path to save the cropped image"
    )
    crop_parser.add_argument(
        "--x",
        type=int,
        required=True,
        help="X-coordinate of the top-left corner for crop",
    )
    crop_parser.add_argument(
        "--y",
        type=int,
        required=True,
        help="Y-coordinate of the top-left corner for crop",
    )
    crop_parser.add_argument(
        "--width", type=int, required=True, help="Width of the crop area"
    )
    crop_parser.add_argument(
        "--height", type=int, required=True, help="Height of the crop area"
    )
